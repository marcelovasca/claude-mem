...
/**
 * Qwen CLI Platform Adapter
 *
 * Normalizes Qwen CLI's hook JSON to NormalizedHookInput.
 * Qwen CLI supports 11 lifecycle hooks; we register 8:
 *
 * Lifecycle:
 ...
 */
export const qwenCliAdapter: PlatformAdapter = {
  normalizeInput(raw) {
    const r = (raw ?? {}) as any;

    // CWD resolution chain: JSON field → env vars → process.cwd()
    const cwd = r.cwd
      ?? process.env.QWEN_CWD
    ...
    if (hookEventName === 'Notification') {
      toolName = toolName ?? 'QwenNotification';
      toolInput = toolInput ?? {
  ...
  formatOutput(result) {
    // Qwen CLI expects: { continue, stopReason, suppressOutput, systemMessage, decision, reason, hookSpecificOutput }
    ...
    if (result.systemMessage) {
      ...
      // Qwen CLI often has issues with ANSI escape sequences in tool output (showing them as raw text)
      const ansiRegex = /[\u001b\u009b][[()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nqry=><]/g;
      output.systemMessage = result.systemMessage.replace(ansiRegex, '');
    }

    // hookSpecificOutput is a first-class Qwen CLI field — pass through directly
